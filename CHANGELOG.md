...
- LLM Scanner: `generate_answer()` and `structured_generate()` accept `context_tools` — additional `ToolInfo` definitions declared alongside the answer tool (but never invoked, since `tool_choice` forces the answer tool). This lets callers pass a `prompt` containing prior `tool_use` blocks (e.g. when asking a structured follow-up question about an existing transcript) without the API rejecting the request for referencing undeclared tools.
