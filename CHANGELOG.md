...
- Observe: OpenAI Chat Completions calls that fail before any usable response now produce a `ModelEvent` instead of being dropped. Covers (a) non-streaming `BadRequestError` / `APIStatusError` (previously the wrapper had no try/except and emitted nothing), and (b) streaming SSE `error` events that fire before the first chunk (previously the wrapper's accumulator was empty and `ChatCompletion.model_validate` rejected it on missing `id`/`model`). Mirrors `inspect_ai.model._openai.openai_handle_bad_request`: a synthetic single-choice output carrying the error message as content, with `stop_reason` inferred from the error code.
