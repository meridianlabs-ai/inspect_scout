# Importing Transcripts – Inspect Scout
...
4.  [Harbor](#harbor): Read transcript data from Harbor agent trajectory (ATIF) files.
...
[Harbor](https://harborframework.com/) is an agent evaluation framework whose installed agents emit [ATIF (Agent Trajectory Interchange Format)](https://github.com/harbor-framework/harbor/blob/main/rfcs/0001-trajectory-format.md) `trajectory.json` files. Scout can import these trajectories, filtering by session ID and/or time range.

To import ATIF trajectories, first install the `harbor` package (requires Python \>= 3.12):

``` bash
pip install "harbor>=0.6.0"
```

Then, use the [atif()](./reference/sources.html.md#atif) source to import trajectories. For example:
...
# Import trajectories from a time range
from datetime import datetime
await db.insert(atif(
    path="path/to/harbor-runs",
...
Note that `from_time`/`to_time` filter on each file’s modification time (`st_mtime`), not the trajectory’s run time — mtime is reset by `cp`, `git checkout`, rsync, etc., so it can diverge from when the run actually occurred.

Note that subagent trajectories are inlined into their parent as spans, so a parent and its referenced subagent files should be imported together in one pass (the default when pointing `path` at a directory). Importing them across separate passes — e.g. incremental time-windowed imports — can store a subagent’s events twice, once standalone and once inlined.

The [atif()](./reference/sources.html.md#atif) source reads trajectories produced by Harbor’s own runner (e.g. `harbor run --agent claude_code`). When Harbor tasks are instead run via Inspect AI through `inspect_harbor`, the output is an Inspect eval log — see [Inspect Logs](#inspect-logs).

ATIF trajectories are mapped to the database schema as follows:

| Field | Value |
|----|----|
| `source_type` | `"atif"` |
| `transcript_id` | Trajectory `trajectory_id`, else `session_id` (or run id) + file stem |
| `source_id` | Trajectory `session_id` (or `trajectory_id`) |
