...
schema. Native session support is a future, separate source — see "Native
...
grows to ~19,647 messages — **90.8M message slots** in total (≈ *N²*/2).
...
Consider whether some agents can be marked as "utility agents". These will
be filtered by the viewer by default, but can optionally be shown.
Heartbeat checks (and perhaps others?) might meet this definition.
Though we don't seem to have any system messages.
