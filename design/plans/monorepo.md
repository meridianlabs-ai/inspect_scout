...
1. **Initialize the submodule once:** `git submodule update --init && cd src/inspect_scout/_view/frontend && pnpm install`
2. **Develop in the submodule:** work on a feature branch inside `src/inspect_scout/_view/frontend/`, using `pnpm dev` for hot reload
3. **Build into the Python repo:** `pnpm build` outputs to `src/inspect_scout/_view/dist/`
4. **Commit both:** `git add src/inspect_scout/_view/dist/ src/inspect_scout/_view/frontend` — the submodule pointer and dist are committed together
...
**How it works.** The submodule is mounted at `src/inspect_scout/_view/frontend/`, so from `_view/frontend/apps/scout/` the Python repo root is six levels up. At build time, Vite checks whether `../../../../../../pyproject.toml` exists and contains `name = "inspect_scout"`. If so, it outputs to that repo's serving path. Otherwise, it defaults to local `dist/`.
...
3. **Wire up the submodule.** Add submodule at `src/inspect_scout/_view/frontend/`. Update `pyproject.toml` artifacts path from `src/inspect_scout/_view/www/dist` to `src/inspect_scout/_view/dist`.
