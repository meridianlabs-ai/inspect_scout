...
- Python repo cloned with submodule initialized and `pnpm install` done (one-time setup):
  ...
  cd src/inspect_scout/_view/frontend && pnpm install
  ```
...
# From src/inspect_scout/_view/frontend/
pnpm types:generate
```
...
cd src/inspect_scout/_view/frontend && git checkout main && git pull && cd ../../../..
