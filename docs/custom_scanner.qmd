...
-   `metadata` --- Whether to read the sample's full metadata, target and scores from the log body (default `True`). Pass `False` if your scanner never reads `transcript.metadata["sample_metadata"]`, `["target"]` or `["scores"]`; the log's summary values are kept instead, and a log with gigabytes of sample metadata costs nothing extra to scan.
