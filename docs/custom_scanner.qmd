...
By default a scanner receives a fully materialized `Transcript`. For eval-log samples larger than 64 MiB, `scan()` can instead hand a scanner a `TranscriptHandle`, which replays messages and events one at a time from an on-disk spool, so peak memory no longer scales with the largest sample in the log.
