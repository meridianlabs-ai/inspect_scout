---
title: Capturing Transcripts
---

::: {.callout-note}
Note that the transcript capture feature described below ais available only in the development version of Inspect Scout. Install the development version from GitHub with:
...
The `observe()` decorator and context manager captures LLM calls and writes transcripts directly to a database. This is useful for recording agent interactions during development and building transcript datasets from custom inference pipelines.


The `observe()` function integrates with Inspect AI's transcript system to automatically capture model events, messages, and token usage. If you are using  [Inspect AI](https://inspect.aisi.org.uk/models.html) for model generations, tool calls, an/or agents, all activity is captured automatically and written to the transcript database. 
...
The `db` parameter accepts either a path string or a `TranscriptsDB` instance. If omitted, it defaults to the project's configured transcripts directory (or `./transcripts` if none is configured).
...
The following `Transcript` fields are automatically populated when a transcript is written:
...
Use `observe_update()` to set transcript fields after execution, which is useful for recording scores or outcomes:
...
You can call `observe_update()` multiple times---fields are merged (metadata is combined, other fields are overwritten).
...
```python
@observe(db="./transcripts", task_set="cybench", model="openai/gpt-4o")
...
1. **Inheritance**: Inner contexts inherit parameters from outer contexts (`task_set`, `model`, etc.)
...
async def main():
    ...
    async def run_case(case: dict) -> None:
        async with observe(task_id=case["id"]):
            ...
            observe_update(score=1.0 if correct else 0.0, success=correct)

    async with observe(db="./transcripts", task_set="math_eval", model="openai/gpt-4o"):
