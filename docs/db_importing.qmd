...
4. [Codex CLI](#codex-cli): Read transcript data from local Codex CLI sessions.
...
[Codex CLI](https://developers.openai.com/codex/cli) is OpenAI's agentic coding tool. Scout can import transcripts directly from Codex's session ("rollout") files, which are stored at `$CODEX_HOME/sessions/` (`~/.codex/sessions/` by default).

Use the `codex()` transcript source to import sessions. Each Codex thread is one rollout file and becomes one transcript. Sub-agent threads spawned via `spawn_agent` are nested inside their parent's transcript; forked threads import as separate transcripts linked via `forked_from_id` metadata; compaction boundaries are preserved as `CompactionEvent`s while messages are stitched across them.
...
async with transcripts_db("s3://my-transcript-db/") as db:
    await db.insert(codex())
```

By default, all sessions in `$CODEX_HOME/sessions` are imported. You can narrow the import by specifying a directory, thread ID, or time range (archived sessions can be included with `include_archived=True`):

```python
# Import sessions from a specific directory
await db.insert(codex(
    path="~/.codex/sessions/2026/08",
))

# Import a specific thread by ID
await db.insert(codex(
...
Codex transcripts are mapped to the database schema as follows:

| Field | Value |
|-------|-------|
| `source_type` | `"codex_cli"` |
| `task_set` | Session working directory (`cwd`) |
| `task_id` | Thread name (from `session_index.jsonl`), else thread ID |
| `agent` | `"codex-cli"` |

Note that imports are additive: a transcript ID that already exists in the database is not re-imported. Because resuming a Codex session appends to the same rollout file (the thread ID is unchanged), turns added after an import will not appear until you re-create the database (e.g. choose **Overwrite** when running `scout db import` interactively).
