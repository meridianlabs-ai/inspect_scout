...
2. [LangSmith](#langsmith): Read transcript data from LangSmith traces or datasets.
...
::: {.callout-note}
The `langsmith()` transcript source described below is available only in the development version of Inspect Scout. Install the development version from GitHub with:
...
- Raw OpenAI API calls (via `wrap_openai`)
- Raw Anthropic API calls (via `wrap_anthropic`)

Use the `langsmith()` transcript source to import traces from a LangSmith project:
...
async with transcripts_db("s3://my-transcript-db/") as db:
    await db.insert(langsmith(project="my-langsmith-project"))
```

You can filter traces by tags, time range, or using LangSmith's filter syntax:
