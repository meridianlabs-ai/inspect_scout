...
3. [Claude Code](#claude-code) and [Codex](#codex): Read transcript data from local coding-agent sessions.
...
[Codex CLI](https://github.com/openai/codex) is OpenAI's terminal coding agent. Scout can import transcripts directly from Codex rollout files, which are normally stored at `~/.codex/sessions/`.

Use the `codex()` transcript source to import rollout sessions. Each rollout file becomes one Scout transcript.
...
By default, all rollout files under `~/.codex/sessions/` are imported. You can narrow the import by specifying a path, session ID, or time range:

```python
# Import rollout files from a specific directory
await db.insert(codex(
    path="~/.codex/sessions/2026/04/13",
))

# Import a specific Codex session by ID
await db.insert(codex(
    session_id="019d8739-0fa5-7a43-9bb7-805000da67af",
))

# Import rollout files from a time range
from datetime import datetime
await db.insert(codex(
...
|-------|-------|
| `source_type` | `"codex"` |
| `task_set` | Rollout working directory (`cwd`) |
| `task_id` | Codex session ID |
| `agent` | `"codex"` |
