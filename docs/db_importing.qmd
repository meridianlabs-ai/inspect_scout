...
Use the `antigravity_cli()` transcript source to import conversations:
...
async with transcripts_db("s3://my-transcript-db/") as db:
    await db.insert(antigravity_cli())
```
...
await db.insert(antigravity_cli(conversation_id="abc123-def456"))
...
await db.insert(antigravity_cli(limit=100))
```
