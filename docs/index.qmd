...
In the example above we used only the `transcript.messages` field. If your scanner needs to access additional metadata about the sample it is processing (e.g. id, epoch, metadata, tags, score, etc.) you can use the `transcript.metadata` field. For example:
