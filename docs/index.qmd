...
We can now run that scanner on our transcripts. The `Scanner` will be called once for each sample trajectory in the log (total samples \* epochs):
...
The `metadata` object (aliased to `m`) provides a convenient way to specified `where()` clauses for filtering transcripts.
...
See the article on [Transcripts](transcripts.qmd) to learn more about the various ways to create, read, filter transcripts.
