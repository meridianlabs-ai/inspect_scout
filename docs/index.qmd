...
See the [Results](results.qmd) article for more details on the columns available in the data frames returned by `scan_results()`.

## Validation

As you are developing scanners you may want to validate them against some ground truth regarding what the ideal scanner result would be. You can do this by including a `ValidationSet` along with your scan. For example, imagine you had a validation set in the form of a CSV with `id` and `target` columns:
...
You can then compute the results from the validation set as you scan:

```python
scan(
    scanners=[ctf_environment()],
    transcripts="./logs",
    validation=validation_set("validation.csv")
)
```
