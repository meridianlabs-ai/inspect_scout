...
Two additional parameters control segmentation behavior:

| Parameter | Description |
|-----------|-------------|
| `context_window` | Override the model's detected context window size (in tokens). |
| `compaction` | `"all"` (default) merges messages across compaction boundaries; `"last"` scans only the final segment after compaction. |
...
To scan timelines instead of raw messages, pass `content=TranscriptContent(timeline=True)` to `llm_scanner()`:

``` python
from inspect_scout import TranscriptContent, llm_scanner, scanner
...
from pydantic import BaseModel, Field
from inspect_scout import (
    AnswerStructured, TranscriptContent, llm_scanner, scanner,
)
...
The composable functions behind `llm_scanner()`—message numbering, message extraction, answer generation, parallel scanning, and result reduction—are available as standalone APIs for building custom scanners. See the [Scanning Toolkit](scanners.qmd#scanning-toolkit) documentation for details.
:::