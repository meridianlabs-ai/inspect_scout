---
title: "LLM Scanner"
llms-description: "Customizable LLM scanner for model evaluation of transcripts"
---
...
| `{{{ metadata }}}` | dict | Additional metadata (includes `sample_metadata`, `target` and `scores` from the log body unless the scanner declines them with `metadata=False`) |
...
async def question_from_transcript(transcript: Transcript) -> str:
    ...
    topic = transcript.metadata["sample_metadata"].get("topic", "unknown")  # needs the default metadata=True
