...
llms-description: "Create custom scanners with the same tools used under the hood by `llm_scanner()`"
---
...
The lowest-level extraction function. It takes a `Timeline`, `TimelineSpan`, or raw `list[Event]`, extracts `ChatMessage` objects from `ModelEvent`s, and handles compaction boundaries---all synchronously, without rendering or segmentation:
