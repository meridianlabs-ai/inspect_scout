...
The `messages_as_str()` function will by default remove system messages from the list. See `MessagesPreprocessor` for other available options.
...
By default, each individual result in the result set will yield its own row in the [results data frame](results.qmd#data-frames). 

When validating scanners that return result sets, you can use [result set validation](validation.qmd#result-set-validation) to specify expected values for each label independently.
