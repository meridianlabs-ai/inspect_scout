...
The `messages_as_str()` function takes a `Transcript` or `list[ChatMessage]` and will by default remove system messages from the message list. See `MessagesPreprocessor` for other available options.
