# Scan Result View Legibility Improvements
...
Use the same label/value class pattern as `TranscriptTitle`:
- Labels: `text-style-label text-size-smallestest text-style-secondary`
- Values: `text-size-small`
...
- The `Column` interface is replaced by `HeadingValue` from `HeadingGrid`
