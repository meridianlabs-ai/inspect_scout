...
- Conversation threads via message fingerprinting
...
def recent_behavior():
    return llm_scanner(
        ...
        answer="boolean",
        compaction="last",  # scan only the most recent context window
    )
```
...
Available static reducers: `ResultReducer.any`, `.mean`, `.median`, `.mode`, `.max`, `.min`, `.union`, `.last`.
...
The `build_timeline()` function converts a flat event list into a hierarchical timeline:

``` python
from inspect_scout import build_timeline, TranscriptContent, transcripts_from

async with transcripts_from("my-project").reader() as reader:
...
In a scanner context, timelines are built automatically when `timeline=True` — you rarely need to call `build_timeline()` directly. It's primarily useful for programmatic exploration of transcript structure.

The builder automatically detects:

- **Top-level structure**: init → solvers → scorers phases
- **Agent spans**: explicit `type="agent"` spans or tool-spawned agents
- **Conversation threads**: via message fingerprinting across events
- **Re-rolled attempts**: discarded retries become `TimelineBranch` objects
- **Utility agents**: single-turn agents with different system prompts
...
Use `filter_timeline()` to create a new timeline containing only spans that match a predicate:

``` python
from inspect_scout import build_timeline, filter_timeline

timeline = build_timeline(events)

# Keep only agent spans
agents_only = filter_timeline(timeline, lambda s: s.span_type == "agent")

# Exclude scorer spans
no_scorers = filter_timeline(timeline, lambda s: s.span_type != "scorers")
```
