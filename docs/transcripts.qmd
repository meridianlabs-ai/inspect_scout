...
::: {.callout-note}
Metadata fields are intended as targets for `.where()` filtering operations and are also forwarded into the results database for scans (`transcript_metadata`). Therefore, you should be careful not to include large amounts of data in these columns. To optimize query and scanning performance, all `metadata` fields are materialized into memory as a DuckDB TABLE when reading from the database.
:::
