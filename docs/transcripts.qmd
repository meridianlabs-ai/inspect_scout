...
To create a transcripts database, use the `transcripts_db()` function to get a `TranscriptsDB` interface and then insert `Transcript` objects:
...
If you are inserting a large number of transcripts you will likely want to read them in a generator that yields transcripts for insertion one at a time. For example, we might implement `read_weave_transcripts()` like this:
...
For example, here is how we might implement `weave_json_to_transcript()`:
