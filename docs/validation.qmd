...
Then use the `validation_set()` function to filter by split when creating your validation set:
...
For example, if your validation expects `label_deception: true` and the scanner returns a result with `label="deception"` and `value={"confidence": 0.9}`, the validation passes because a non-empty dict is a positive value.

If your validation expects `label_phishing: false` and the scanner returns no results with `label="phishing"`, validation passes because absence is treated as negative.
...
Add a `predicate` column to specify comparison logic for individual cases:
