---
title: Workflow
llms-description: "End to end workflow for transcript analysis projects"
---
...
See the article on [Projects](projects.qmd) to learn more about using projects.
...
![](images/transcript-messages-copy.png){.border}
...
For long-running scans it can be useful to inspect partial results while the scan is still in flight—for example to sanity check that a scanner is behaving as expected before committing to a multi-hour run. By default results are only written to the scan location when the scan completes (or is interrupted).

Use the `--results-buffer` option (or the `results_buffer` scan/scan-job/project option) to periodically sync in-progress results to the scan location every N recorded results:

``` bash
scout scan cybench_scan.py --results-buffer 100
```

With this set, the compacted scanner Parquet files at the scan location are refreshed every 100 results, so you can point `scan_results_df()` (or Scout View) at the scan directory and analyze what's been produced so far. Writes are atomic, so readers always observe a consistent snapshot. Periodic syncing adds some I/O overhead, so choose N to balance freshness against cost (larger values sync less often); leaving it unset disables periodic syncing entirely.
