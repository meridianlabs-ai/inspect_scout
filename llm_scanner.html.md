...
When scanning for different categories cases it may be more natural for
the scanner to return multiple results. To do this, add a `label` field
and specify `result_set=True`. For example:
...
def cyberlint():
    return llm_scanner(
        ...
        """),
        answer=AnswerStructured(type=CyberLint, result_set=True)
    )
```

Note that we add a `label` field for distinguishing different
observation types and pass `result_set=True` to indicate we want to
prompt the model to generate multiple results.

### Result Values

Scanners produce results which ultimately carry a specific `value`. By
default, structured scans produce a `value` of `True`. This is suitable
for when scanners are simply detecting the presence or absence of
something.

1.  For cases where you want your scanner to yield a more specific
