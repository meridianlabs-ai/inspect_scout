...
- Prompt templates with the ability to customize the core instructions,
  explanation, and answer provided by the scanner.
...
See the section below for more details on how prompts are constructed
for `llm_scanner()`. See the following sections below for the specific
prompts generated for various answer types: [boolean](#answer-boolean),
[numeric](#answer-numeric), [string](#answer-string),
[labels](#answer-labels).

## Prompt Template

Here is the structure of the prompt template for `llm_scanner()` (note
that prompt templates are processed using
...
| `{{ answer_format }}` | Formatting guidelines used by the current answer type (e.g. “‘ANSWER: xxx’ (without quotes) where xxx is yes or no.”) |
...
The following provides an end to end example of a scanner, the generated
prompt, and the results yielded back from the scanner.
...
Messages are included within the prompt template by calling the
`messages_to_str()` function, passing any `ContentFilter` passed to
`llm_scanner()`. The content filter includes the following fields:
