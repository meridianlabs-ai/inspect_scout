...
|  |  |
|----|----|
| [list](#scout-trace-list) | List all trace files. |
| [dump](#scout-trace-dump) | Dump a trace file to stdout (as a JSON array of log records). |
| [http](#scout-trace-http) | View all HTTP requests in the trace log. |
| [anomalies](#scout-trace-anomalies) | Look for anomalies in a trace file (never completed or cancelled actions). |
