...
Enabled by setting ``SCOUT_S3_MONITOR=1`` (or ``true``/``yes``). When
...
This is a diagnostics-only hack: rather than refactoring the bare
``AsyncFilesystem()`` construction sites in scout to share a single
instance, we just attach hooks to every client that gets created. Per-
process scope (workers patch themselves on first call).
...
_ENABLED_ENV = "SCOUT_S3_MONITOR"


def s3_monitor_enabled() -> bool:
    """True if the SCOUT_S3_MONITOR env var is set to a truthy value."""
...
async def monitor_s3_traffic(
    *, interval: float = 1.0, label: str = ""
    ...
    """No-op unless SCOUT_S3_MONITOR is set.
