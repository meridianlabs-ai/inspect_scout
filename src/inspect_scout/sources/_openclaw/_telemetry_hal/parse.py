...
class OpenClawTelemetry:
    ...
    ``usage``, ``timestamp``, ``model``). ``compactions`` are the real
    ``compactionSummary`` turns. ``result_by_callid`` maps a toolCall id to its
...
def _consolidate(raw_events: Iterable[dict[str, Any]]) -> _Consolidated:
    ...
    1. Dedup/partition over ``agent.*`` cumulative snapshots. Assistant turns are
       ...
       kind), ``toolResult``s by ``toolCallId``, ``compactionSummary``s, and
       orchestrator-only ``user`` turns. Turns and user prompts with no
    ...
    for ev in raw_events:
        ...
        if out.session_id is None and is_agent and kind in ORCH_KINDS:
        ...
        if is_agent:
            for m in ev.get("messages") or []:
                ...
                if role == "assistant":
                    ...
                    if rid is None:
                        # No responseId (e.g. failed-before-content turns). Key by
                        # (timestamp, content) so the SAME turn re-dumped across
                        # cumulative snapshots collapses to one.
                        rid = (
                            "a",
                            m.get("timestamp"),
                ...
                elif role == "user" and kind in ORCH_KINDS:
