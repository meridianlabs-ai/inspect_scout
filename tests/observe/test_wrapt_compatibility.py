"""SDK-backed tests for the observe capture proxies.

Real OpenAI/Anthropic SDK stream objects over mocked HTTP, wrapped by the
capture proxies the providers install, plus the ObjectProxy attribute
isolation and unwrapping those proxies rely on.
"""
...
from inspect_scout._observe.providers.provider import ObjectProxyBase
...
def test_openai_sync_sdk_stream(api: Literal["chat", "responses"]) -> None:
    ...
    with openai.OpenAI(
        ...
        if api == "chat":
            ...
            proxy = OpenAIChatStreamCapture(stream, {}, captures.append)
...
async def test_openai_async_sdk_stream(api: Literal["chat", "responses"]) -> None:
    ...
    async with openai.AsyncOpenAI(
        ...
        if api == "chat":
            ...
            proxy = OpenAIChatAsyncStreamCapture(stream, {}, captures.append)
            assert proxy.__wrapped__ is stream
...
class _CounterProxy(ObjectProxyBase):
    value: int
    _self_marker: str
