"""Tests for Phoenix event conversion."""

import pytest
from inspect_ai.event import ModelEvent, SpanBeginEvent, SpanEndEvent, ToolEvent
from inspect_scout.sources._phoenix.events import (
    spans_to_events,
    to_model_event,
    to_span_begin_event,
    to_span_end_event,
...
class TestToModelEvent:
    ...
    async def test_model_event_span_id(self) -> None:
        ...
        span = create_openai_llm_span(parent_id="parent-123")
        event = await to_model_event(span)
...
class TestToSpanEvents:
    ...
    def test_span_begin_event(self) -> None:
        """Convert chain span to SpanBeginEvent."""
        span = create_chain_span(span_id="chain-1", name="agent-run")
        event = to_span_begin_event(span)

        assert isinstance(event, SpanBeginEvent)
        assert event.id == "chain-1"
        assert "agent-run" in event.name

    def test_span_end_event(self) -> None:
        """Convert chain span to SpanEndEvent."""
        span = create_chain_span(span_id="chain-1")
...
class TestSpansToEvents:
    ...
    async def test_convert_mixed_spans(self) -> None:
        ...
        chain_span = create_chain_span(span_id="chain-1")

        events = await spans_to_events([llm_span, tool_span, chain_span])
    ...
    async def test_tool_call_trace_events(self) -> None:
        ...
        assert len(model_events) == 2  # Two LLM calls
        assert len(tool_events) == 1  # One tool execution
        assert len(span_begin_events) == 1  # Chain span
