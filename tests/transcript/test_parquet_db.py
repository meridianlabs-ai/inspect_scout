...
async def test_mixed_content_encodings_read_identically(test_location: Path) -> None:
    ...
    db = ParquetTranscriptsDB(str(test_location))
    await db.connect()
    await db.insert(create_test_transcripts(6))
    await db.commit()
    await db.disconnect()
    ...
    for transcript_id, digest in rows:
        by_id.setdefault(transcript_id, set()).add(digest)
    assert all(len(digests) == 1 for digests in by_id.values())
