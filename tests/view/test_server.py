...
def app_with_results_dir(tmp_path: Path) -> TestClient:
    ...
    app = view_server_app(results_dir=results_dir)
    return TestClient(app)


class TestDfToIpc:
    """Tests for the df_to_ipc function."""

    def test_df_to_ipc_basic(self, sample_dataframe: pd.DataFrame) -> None:
        """Test basic DataFrame to IPC conversion."""
        result = df_to_ipc(sample_dataframe)

        assert isinstance(result, IPCDataFrame)
        assert result.format == "arrow.feather"
    ...
    def test_df_to_ipc_roundtrip(self, sample_dataframe: pd.DataFrame) -> None:
        """Test that IPC conversion can be reversed."""
        ipc_df = df_to_ipc(sample_dataframe)

        # Decode and deserialize
        assert ipc_df.data is not None
        decoded = base64.b64decode(ipc_df.data)
    ...
    def test_df_to_ipc_empty_dataframe(self) -> None:
        """Test conversion of empty DataFrame."""
        empty_df = pd.DataFrame({"col1": [], "col2": []})
        result = df_to_ipc(empty_df)
...
class TestIPCSerializableResults:
    """Tests for IPCSerializableResults."""

    def test_ipc_serializable_results_creation(
        self, sample_scan_spec: ScanSpec, sample_dataframe: pd.DataFrame
    ) -> None:
        """Test creation of IPCSerializableResults."""
        ipc_df = df_to_ipc(sample_dataframe)
        scanners = {"scanner1": ipc_df}

        result = IPCSerializableResults(
            complete=True,
            spec=sample_scan_spec,
            location="/test/scan",
            summary=Summary(scanners={}),
        ...
        assert result.location == "/test/scan"
        assert "scanner1" in result.scanners
        assert isinstance(result.scanners["scanner1"], IPCDataFrame)
...
class TestViewServerAppScansEndpoint:
    ...
    async def test_scans_endpoint_success(
        ...
        with patch(
            "inspect_scout._view.server.scan_list_async", return_value=mock_scans
        ):
    ...
    async def test_scans_endpoint_no_results_dir(self) -> None:
        ...
        app = view_server_app(results_dir=None)
...
class TestViewServerAppScanDfEndpoint:
    ...
    async def test_scanner_df_endpoint_success(
        ...
        with patch(
            "inspect_scout._view.server.scan_results_arrow_async",
...
class TestViewServerAppScanEndpoint:
    ...
    async def test_scan_endpoint_success(
        ...
        with patch(
            "inspect_scout._view.server.scan_results_df_async",
            return_value=mock_results,
        ):
...
class TestViewServerAppScanDeleteEndpoint:
    ...
    async def test_scan_delete_endpoint_success(
        ...
        with patch("inspect_scout._view.server.remove_scan_results") as mock_remove:
...
class TestAuthorizationMiddleware:
    ...
    def test_authorization_middleware_success(self) -> None:
        ...
        app = view_server_app(results_dir="/test")
        ...
        with patch("inspect_scout._view.server.scan_list_async", return_value=[]):
            response = client.get(
...
class TestAccessPolicy:
    ...
    async def test_access_policy_read_forbidden(self) -> None:
        ...
        app = view_server_app(access_policy=mock_access_policy, results_dir="/test")
        client = TestClient(app)

        with patch("inspect_scout._view.server.scan_results_df_async") as mock_scan:
    ...
    async def test_access_policy_list_forbidden(self) -> None:
        ...
        with patch("inspect_scout._view.server.scan_list_async") as mock_list:
...
class TestMappingPolicy:
    ...
    async def test_mapping_policy_map_and_unmap(self) -> None:
        ...
        app = view_server_app(mapping_policy=mock_mapping_policy, results_dir="/test")
        client = TestClient(app)
