...
Use `@validation_predicate` when a custom predicate must be recorded and recreated for scan resume:
...
validation = validation_set(
    "validation.csv",
    predicate=within_range(distance=5),
)
```

Scout records the registered predicate’s name and Inspect registry-compatible creation parameters, not its Python implementation. Use the same parameter conventions as other Inspect extensions: ordinary serializable values and registered Inspect objects or models are portable, while arbitrary Python objects are not. Non-finite floating-point values are rejected rather than silently changed in the scan artifact. The predicate’s module or package must be available when the scan is resumed.

If the predicate is defined in a local source file rather than an installed package, Scout records the file path and loads that file when the scan is resumed (the same way it loads the source file of a scanner defined outside a package). Listing or viewing a scan never loads predicate code.

An unregistered async callable can still be used for the current scan:
...
validation = validation_set(
    "validation.csv",
    predicate=normalized_equal,
)
```

Unregistered callables are not stored in the portable scan specification. Resume from Python by supplying the trusted callable again:
...
scan_resume(
    scan_location,
    predicate_overrides={
        "my_scanner": normalized_equal,
    },
)
```

Completed scans remain viewable without loading predicate code. A scan with an unregistered or legacy custom predicate cannot be resumed by location alone (including with `scout scan resume`), and Scout never substitutes a missing predicate with equality.
